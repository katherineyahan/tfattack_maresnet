...
from utils import get_network, get_training_dataloader, get_valid_dataloader, get_test_dataloader, \
    most_recent_folder, best_auc_weights, get_parameter_number, save_best_result_attack
...
def main(args) :
    ...
    dataset_list = sorted([d for d in os.listdir(dataset_path) if os.path.isdir(os.path.join(dataset_path, d))])
    ...
    for dataset_ in dataset_list:
        ...
        #train_scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, patience = 25)
        #train_scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=settings.EPOCH_TRANSFORM)
        #train_scheduler = optim.lr_scheduler.CyclicLR(optimizer, base_lr=args.lr, max_lr=0.1)
        ...
        if args.resume:
            ...
            if best_weights:
                ...
                loss_valid, acc_valid, best_auc, pred_result_valid = eval_training(net, dna_valid_loader, loss_function, softmax_output, args, train_after=False)
                # save best result
        ...
        for epoch in range(1, settings.EPOCH_TRANSFORM + 1):
            ...
            log_dic = train(net, dna_training_loader, optimizer, loss_function, epoch, args,
                            output_interval, args.attack)

            if epoch > args.warm:
                train_scheduler.step()

            loss_valid, acc_valid, auc_valid, pred_result_valid = eval_training(net, dna_valid_loader,
